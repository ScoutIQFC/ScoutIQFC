...
.dots-btn { background: transparent !important; border: none !important;
    color: rgba(255,255,255,0.3) !important; font-size: 14px !important;
    cursor: pointer; padding: 0 4px !important; line-height: 1;
    width: auto !important; min-width: 0 !important; }
.dots-btn:hover { color: rgba(255,255,255,0.8) !important; }
.sb-dots .stButton > button {
    background: transparent !important; border: none !important;
    color: rgba(255,255,255,0.25) !important; font-size: 13px !important;
    padding: 2px 6px !important; width: auto !important;
    min-width: 0 !important; box-shadow: none !important;
    margin: 0 !important; border-radius: 3px !important;
    line-height: 1 !important;
}
.sb-dots .stButton > button:hover {
    color: rgba(255,255,255,0.8) !important;
    background: rgba(255,255,255,0.08) !important;
}
.dot-action-edit .stButton > button {
...
.dot-action-del .stButton > button {
...
with st.sidebar:
    ...
    if st.session_state.mode == "youth":
        ...
        for club, cplayers in clubs.items():
            ...
            # Academy row: expand button + three-dot menu
            ...
            if st.session_state.get("open_menu") == f"acad_{club}":
                with st.container():
                    ...
                    st.markdown('<div class="dot-action-edit">', unsafe_allow_html=True)
                    if st.button("✏  Edit Academy", key=f"edit_acad_{club}"):
                        ...
                        st.session_state.selected_player_id = None
                        st.rerun()
                    st.markdown('</div><div class="dot-action-del">', unsafe_allow_html=True)
            ...
            if st.session_state.get("confirm_del_acad") == club:
                st.markdown(f'<div style="background:rgba(239,68,68,0.08);border:1px solid rgba(239,68,68,0.2);border-radius:6px;padding:7px 12px;margin:2px 14px;font-size:11px;color:#fca5a5;">Delete {club} and all players?</div>', unsafe_allow_html=True)
                ...
                with cc1:
                    if st.button("Yes", key=f"cda_{club}"):
                        for dp in cplayers:
                            ...
                            cursor.execute("DELETE FROM report_edits WHERE player_id=?", (dp[0],))
                    ...
                    if st.session_state.get("confirm_del_player_sb") == p[0]:
                        ...
                        with dc1:
                            if st.button("Yes", key=f"cdp_{p[0]}"):
                                ...
                                cursor.execute("DELETE FROM report_edits WHERE player_id=?", (p[0],))
...
if st.session_state.mode == "youth":
    ...
    with hc2:
        ...
        if st.button("← Academy", key=f"bk_acad_{pid}"):
            st.session_state.selected_academy = pl[6]
            st.session_state.selected_player_id = None
            st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)
    with hc3:
        st.markdown('<div style="padding-top:32px;">', unsafe_allow_html=True)
        if st.button("⋮", key=f"profile_dots_{pid}"):
            cur = st.session_state.get("open_menu")
            st.session_state["open_menu"] = None if cur == f"profile_{pid}" else f"profile_{pid}"
            st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)
    if st.session_state.get("open_menu") == f"profile_{pid}":
        ma, mb, _ = st.columns([1, 1, 4])
        with ma:
            st.markdown('<div class="dot-action-edit">', unsafe_allow_html=True)
            if st.button("✏ Edit", key=f"prof_edit_{pid}"):
                st.session_state["open_menu"] = None
                st.session_state.show_edit_player = True
                st.session_state.edit_player_id = pid
                st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)
        with mb:
            st.markdown('<div class="dot-action-del">', unsafe_allow_html=True)
            if st.button("✕ Delete", key=f"prof_del_{pid}"):
                st.session_state["open_menu"] = None
                st.session_state["confirm_del_profile"] = pid
                st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)
    if st.session_state.get("confirm_del_profile") == pid:
        st.warning(f"Permanently delete {pl[1]} and all their data?")
        pd1, pd2, _ = st.columns([1, 1, 4])
        with pd1:
            if st.button("Yes, delete", key=f"conf_del_prof_{pid}"):
                cursor.execute("DELETE FROM sessions WHERE player_id=?", (pid,))
                cursor.execute("DELETE FROM reports WHERE player_id=?", (pid,))
                cursor.execute("DELETE FROM report_edits WHERE player_id=?", (pid,))
                cursor.execute("DELETE FROM players WHERE id=?", (pid,))
                conn.commit()
                st.session_state["confirm_del_profile"] = None
                st.session_state.selected_player_id = None
                st.rerun()
        with pd2:
            if st.button("Cancel", key=f"canc_del_prof_{pid}"):
                st.session_state["confirm_del_profile"] = None; st.rerun()
...
Or type rough notes: Erling Haaland, 24, striker Man City, 27 goals 5 assists this season""",
        ...
        # ── PARSED RESULTS (persists across reruns via session state) ──
        ...
        if st.session_state.get("pro_ai_parsed"):
            ...
            sv_col, disc_col, _ = st.columns([1,1,4])
                ...
                if st.button("✅ Save to Platform", key="pro_ai_confirm"):
                    ...
                    for p in pro_players:
                        ...
                        try:
                            ...
                            for gi in range(total_goals):

                                goal_list[random.randint(0, appearances-1)] += 1

                            for ai2 in range(total_assists):
                            ...
                            for gw in range(1, appearances+1):
                                ...
                                cursor.execute(
                                    ...
                                    random.randint(25,55), random.randint(1,4),
                            ...
                            st.warning(f"Skipped {p.get('name','?')}: {e}")
                    ...
                    st.success(f"✅ {saved} player(s) saved — opening player report...")
