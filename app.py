...
def ai_clean_data(raw_text, data_type="youth"):
    ...
    if data_type == "youth":
        ...
        # Build a clean extraction prompt - NO f-string to avoid template confusion
        prompt = (
            "You are a professional football data analyst. Extract a complete player profile from the raw text below.\n\n"
            "IMPORTANT: The raw text contains a stats table with COLUMNS. Read it carefully row by row.\n"
            "The table format is: Season | Age | Squad | Country | Comp | LgRank | MP | Starts | Min | 90s | Gls | Ast | ...\n\n"
            "EXTRACTION STEPS:\n"
            "Step 1 - Player identity:\n"
            "  - name: the player the page is about (first line usually has it, e.g. 'Bukayo Saka')\n"
            "  - team: current club (e.g. 'Arsenal')\n"
            "  - position: from the page (e.g. 'Right Winger' maps to 'Forward')\n"
            "  - dob: date of birth if shown (convert to YYYY-MM-DD)\n"
            "  - age: current age as integer\n\n"
            "Step 2 - CURRENT SEASON (find the row with the HIGHEST year, e.g. 2025-2026):\n"
            "  - season: e.g. '2025-26'\n"
            "  - appearances: the MP column value for that row (NOT career total)\n"
            "  - minutes: the Min column value for that row (NOT career total)\n"
            "  - goals: the Gls column value for that row (NOT career total, NOT xG)\n"
            "  - assists: the Ast column value for that row (NOT career total, NOT xA)\n"
            "  - yellow_cards: CrdY column for that row\n"
            "  - red_cards: CrdR column for that row\n\n"
            "Step 3 - SHOT STATS (usually a separate mini-table with columns: apps, mins, goals, xg, shots, sot, conv%):\n"
            "  - xg: the xG value (e.g. 6.99) - this is expected goals, NOT actual goals\n"
            "  - shots: total shots (e.g. 63)\n"
            "  - shots_on_target: shots on target (e.g. 26)\n"
            "  - conversion_rate: conversion % (e.g. 9.52)\n"
            "  - xa: xA value if present, else 0\n\n"
            "Step 4 - CAREER TOTALS (find the summary row that says '8 Seasons' or 'Total' or shows career sums):\n"
            "  - career_appearances: total MP across all seasons\n"
            "  - career_minutes: total Min across all seasons\n"
            "  - career_goals: total Gls across all seasons\n"
            "  - career_assists: total Ast across all seasons\n\n"
            "Step 5 - PEAK SEASON (scan all rows, find which season had the most goals):\n"
            "  - peak_season: e.g. '2023-24'\n"
            "  - peak_goals: goals in that peak season\n"
            "  - peak_assists: assists in that peak season\n\n"
            "Step 6 - coach_notes: Write a 3-4 sentence summary of the player's career arc and key stats.\n\n"
            "Return ONLY a JSON array containing ONE object with ALL these fields:\n"
            ...
            "Use 0 for any missing numeric value. No markdown. No explanation. JSON array only.\n\n"
            "RAW DATA TO PARSE:\n"
        ) + raw_text
