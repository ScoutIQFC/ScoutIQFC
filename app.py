...
def check_password():
    ...
    def password_entered():
        ...
        # Beta accounts - full youth access
            ...
            st.session_state["beta_badge"] = True
...
# Beta users now have full access to youth portal
...
if st.session_state.mode == "youth":
    ...
    if st.session_state.get("show_edit_player") and st.session_state.get("edit_player_id"):
        ...
        if ep_edit:
            ...
            with ep_tab2:
                ...
                # AI Import for player sessions

                st.markdown('<div class="section-title" style="font-size:15px;margin-top:16px;">✨ AI Import Session Data</div>', unsafe_allow_html=True)

                st.markdown('<div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;padding:10px 14px;margin-bottom:10px;font-size:12px;color:#1d4ed8;">Paste any match notes, stats, or data. Claude will structure it and add it as session records.</div>', unsafe_allow_html=True)

                ai_sess_file = st.file_uploader("Upload file (PDF, Word, Excel, CSV, image)", type=["jpg","jpeg","png","pdf","docx","xlsx","csv","txt"], key=f"ai_sess_file_{ep_edit[0] if ep_edit else pid}_edit", label_visibility="collapsed")

                ai_sess_text = st.text_area("Or paste / type any data", placeholder="e.g. Match vs Northgate: 45 mins, 2 goals, 1 assist, 8 sprints, great attitude. Or paste a copied stats table.", height=80, key=f"ai_sess_text_{ep_edit[0] if ep_edit else pid}_edit", label_visibility="collapsed")

                if st.button("✨ Parse & Add Sessions", key=f"ai_sess_parse_{ep_edit[0] if ep_edit else pid}_edit"):

                    input_text = ai_sess_text.strip() if ai_sess_text else ""

                    if ai_sess_file:

                        file_bytes = ai_sess_file.read()

                        ext = ai_sess_file.name.split(".")[-1].lower()

                        if ext in ["jpg","jpeg","png","webp"]:

                            import base64 as _b64s

                            b64 = _b64s.standard_b64encode(file_bytes).decode()

                            cl_s = anthropic.Anthropic(api_key=get_api_key())

                            rs = cl_s.messages.create(model="claude-opus-4-5", max_tokens=2000,

                                messages=[{"role":"user","content":[{"type":"image","source":{"type":"base64","media_type":"image/jpeg","data":b64}},{"type":"text","text":"Extract football session data from this image as JSON array with: session_date, goals, assists, minutes_played, distance_covered_km, sprint_count, coachability_rating, attitude_score, coach_notes. Return ONLY JSON array."}]}])

                            input_text = rs.content[0].text

                            raw_result = input_text

                        elif ext == "docx":

                            from docx import Document as _Ds

                            try: input_text = "\n".join([p.text for p in _Ds(io.BytesIO(file_bytes)).paragraphs if p.text.strip()])

                            except: input_text = file_bytes.decode("utf-8","ignore")[:5000]

                            raw_result = ai_clean_data(input_text, "youth")

                        elif ext in ["xlsx","xls"]:

                            raw_result = ai_clean_data(pd.read_excel(io.BytesIO(file_bytes)).to_string(index=False)[:4000], "youth")

                        else:

                            raw_result = ai_clean_data(file_bytes.decode("utf-8","ignore")[:5000], "youth")
                        ...
                        st.warning("Add some data first."); st.stop()
                    ...
                    try:
                        ...
                        parsed_s = _js.loads(raw_result.strip())

                        if not isinstance(parsed_s, list): parsed_s = [parsed_s]

                        target_pid = ep_edit[0] if ep_edit else pid

                        added = 0

                        for s in parsed_s:

                            def _sv(k, d=0, fl=False):

                                try:

                                    v = s.get(k, d)

                                    return float(v) if fl else int(float(v)) if v is not None else d

                                except: return d

                            cursor.execute("INSERT INTO sessions (player_id,session_date,session_type,minutes_played,distance_covered_km,sprint_count,top_speed_kmh,passes_completed,passes_attempted,dribbles_completed,defensive_actions,goals,assists,chances_created,tackles_won,coachability_rating,attitude_score,consistency_rating,coach_notes) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",

                                (target_pid, str(s.get("session_date","2024-09-01"))[:10], s.get("session_type","match"),

                                _sv("minutes_played",90), _sv("distance_covered_km",8,True), _sv("sprint_count",15),

                                _sv("top_speed_kmh",28,True), _sv("passes_completed",25), _sv("passes_attempted",32),

                                _sv("dribbles_completed",4), _sv("defensive_actions",6),

                                _sv("goals"), _sv("assists"), _sv("chances_created"), _sv("tackles_won"),

                                _sv("coachability_rating",7), _sv("attitude_score",7), _sv("consistency_rating",7),

                                str(s.get("coach_notes",""))))

                            conn.commit(); added += 1

                        st.success(f"✅ {added} session(s) added!"); st.rerun()
    ...
    sh_c1, sh_c2, sh_c3 = st.columns([5, 1, 1])
    ...
    with sh_c3:
        ...
        ai_sess_key = f"show_ai_sess_{pid}"

        if ai_sess_key not in st.session_state: st.session_state[ai_sess_key] = False

        if st.button("✨ AI", key=f"ai_s_{pid}"):

            st.session_state[ai_sess_key] = not st.session_state[ai_sess_key]; st.rerun()

        st.markdown('</div>', unsafe_allow_html=True)

    if st.session_state.get(ai_sess_key):

        with st.expander("✨ AI Import — paste match notes or stats", expanded=True):

            st.markdown('<div style="font-size:12px;color:#6b7280;margin-bottom:8px;">Paste any match data, notes, or stats. Claude will extract session data and add it to this player.</div>', unsafe_allow_html=True)

            ai_sess_file = st.file_uploader("Upload file (optional)", type=["jpg","jpeg","png","pdf","docx","xlsx","csv","txt"], key=f"ai_sess_file_{pid}", label_visibility="collapsed")

            ai_sess_text = st.text_area("Or type/paste data", placeholder="e.g. Marcus played 70 mins vs City, scored 2, covered 9.2km, 32 sprints, great attitude, needs work on passing", height=80, key=f"ai_sess_text_{pid}", label_visibility="collapsed")

            if st.button("✨ Parse & Add Session", key=f"ai_sess_parse_{pid}"):

                raw = ai_sess_text.strip() if ai_sess_text else ""

                if ai_sess_file and not raw:

                    raw = ai_sess_file.read().decode("utf-8","ignore")[:4000]
                ...
                if raw:
                    ...
                    with st.spinner("Parsing..."):
                        ...
                        try:
                            ...
                            result = ai_clean_data(raw, "youth")

                            result = result.strip()

                            if result.startswith("```"):

                                parts = result.split("```"); result = parts[1][4:].strip() if parts[1].startswith("json") else parts[1].strip()

                            parsed_sess = _json.loads(result)

                            if not isinstance(parsed_sess, list): parsed_sess = [parsed_sess]

                            added = 0

                            for sp in parsed_sess:

                                def sv(k,d=0,fl=False):

                                    try: v=sp.get(k,d); return float(v) if fl else int(float(v)) if v is not None else d

                                    except: return d

                                cursor.execute("INSERT INTO sessions (player_id,session_date,session_type,minutes_played,distance_covered_km,sprint_count,top_speed_kmh,passes_completed,passes_attempted,dribbles_completed,defensive_actions,goals,assists,chances_created,tackles_won,coachability_rating,attitude_score,consistency_rating,coach_notes) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",

                                    (pid,sp.get("session_date","2025-01-01"),sp.get("session_type","match"),sv("minutes_played",70),sv("distance_covered_km",8,True),sv("sprint_count",15),sv("top_speed_kmh",28,True),sv("passes_completed",25),sv("passes_attempted",35),sv("dribbles_completed",3),sv("defensive_actions",5),sv("goals"),sv("assists"),sv("chances_created"),sv("tackles_won"),sv("coachability_rating",7),sv("attitude_score",7),sv("consistency_rating",7),str(sp.get("coach_notes",""))))

                                conn.commit(); added += 1

                            st.session_state[ai_sess_key] = False

                            st.success(f"{added} session(s) added!"); st.rerun()

                        except Exception as e:

                            st.error(f"Could not parse: {str(e)[:80]}")
        ...
        if st.button("＋ Add", key=f"add_s2_{pid}"):
                ...
                if st.button("Save to Platform", key="pro_ai_confirm"):
                    ...
                    for p in pro_players:
                        ...
                        try:
                            ...
                            last_epid = epid
                            last_epid = epid
                            # Store ONE accurate season record - actual data, no distribution
                            total_shots = int(p.get("shots", 0) or 0)
                            total_sot = int(p.get("shots_on_target", 0) or 0)
                            total_passes = int(p.get("passes_completed", 0) or 0)
                            total_tackles = int(p.get("tackles", 0) or int(p.get("total_tackles", 0) or 0))
                            rc = int(p.get("red_cards", 0) or 0)
                            season_rating = round(min(9.9, 6.0 + goals_p90 * 1.5 + assists_p90 * 1.0), 1)
                            season_label = str(p.get("season", "Season")) or "Season"
                            cursor.execute(
                                "INSERT INTO epl_sessions (player_id,gameweek,opponent,minutes_played,goals,assists,shots,shots_on_target,xg,xa,passes_completed,tackles_won,yellow_cards,red_cards,rating) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                                (epid, 1, season_label, total_mins,
                                 total_goals, total_assists, total_shots, total_sot,
                                 xg_total, xa_total, total_passes, total_tackles,
                                 yellow_cards, rc, season_rating)
                            )
                            conn.commit(); saved += 1
